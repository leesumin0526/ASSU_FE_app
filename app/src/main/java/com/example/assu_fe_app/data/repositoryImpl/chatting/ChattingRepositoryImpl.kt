...
class ChattingRepositoryImpl @Inject constructor(
    ...
    {
        override suspend fun createChatRoom(request: CreateChatRoomRequestDto): CreateChatRoomModel {
            return apiHandler({api.createChatRoom(request)}) {response: ApiResult<CreateChatRoomResponseDto> -> response.toModel()}
        }
    }